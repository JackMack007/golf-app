...
exports.handler = async function(event, context) {
  console.log('Handling users request:', event.path, event.httpMethod);
  ...
  if (event.httpMethod === 'OPTIONS') {
    console.log('Handling OPTIONS preflight request for users');
    return {
      statusCode: 200,
      headers: corsHeaders,
      body: ''
  ...
  try {
    const path = event.path
      .replace(/^\/\.netlify\/functions\/users\/?/, '/api/')
      .replace(/^\/api\/api\/?/, '/api/')
      .replace(/^\/+/, '/')
      .replace(/\/+$/, '');
    ...
    console.error('Error in users handler:', error.message, error.stack);
    return {
      statusCode: 500,
      headers: corsHeaders,
